        ...
        <div class="form-group additional-data d-none col-md-6">
            ...
            <select class="form-control" aria-label="Default select example" id="selectColor" name="color_id">
                ...
                @foreach(\App\Models\Color::all() as $color)
                    <option value="{{ $color->id }}" color="{{ $color->color }}" @if($product->color_id == $color->id) selected @endif>
                        {{$color->{'name_' . LaravelLocalization::getCurrentLocale()} }}
                    </option>
                @endforeach
            </select>
            <span id="colorField" style="border-radius: 4px; background-color: @if(!is_null($product->color_id)) {{ $product->color->color }} @else white @endif; display: block; width: 50px; height: 50px; margin-top: 10px"></span>
            @error('color_id')
        ...
        <div class="form-group additional-data d-none col-md-6">
            ...
            <select class="form-control" aria-label="Default select example" id="selectTrademark" name="trademark_id">
                ...
                @foreach(\App\Models\TradeMark::all() as $trademark)
                    <option value="{{ $trademark->id }}" trademark_logo="{{ asset('storage/' . $trademark->logo) }}" trademark_alt="{{ $trademark->name . ' logo' }}" @if($product->trademark_id == $trademark->id) selected @endif>
                        {{$trademark->{'name_' . LaravelLocalization::getCurrentLocale()} }}
                    </option>
                @endforeach
            </select>
            <img src="@if(!is_null($trademark->logo)){{ asset('storage/'.$trademark->logo) }} @endif" alt="" class="@if(is_null($trademark->logo)) d-none @endif mt-2" id="trademark_logo" style="width: 50px; height: 50px; border-radius: 4px">
            @error('trademark_id')
            <div class="invalid-feedback d-block">{{ $message }}</div>
            @enderror
        </div>
        <div class="form-group additional-data d-none col-md-6">
            ...
            <select class="form-control" aria-label="Default select example" id="selectColor" name="manufacture_id">
                ...
                @foreach(\App\Models\Manufact::all() as $manufact)
                    <option value="{{ $manufact->id }}" @if($product->manufacture_id == $manufact->id) selected @endif>
